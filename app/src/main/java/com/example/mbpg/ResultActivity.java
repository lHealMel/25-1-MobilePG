...
public class ResultActivity extends AppCompatActivity {
    ...
    // GeminiService에 MBTI를 전달하고, MBTI에 따른 음악 추천 정보를 가져오고 UI에 표시합니다.
    // 추천 곡들을 받으면 각 곡에 대해 Spotify 트랙 정보를 조회합니다.
    ...
    // 검색용 artist 이름과 노래 이름을 받아서 spotify API에 전달하고, 곡에 대한 해당 URI를 받아옵니다.
    // URI는 Spotify SDK로만 사용이 가능합니다.
    private void fetchSpotifyTrackInfo(String artistForSearch, String song) {
        SpotifyService.geturifrommbti(artistForSearch, song, new SpotifyResultCallback() {
            ...
            public void onSuccess(String trackUri, String trackName, String artistName, String albumArtUrl, String previewUrl, boolean isPlayable) {
                ...
                if (isPlayable) {
                    // 트랙이 재생 가능한 경우
                    if (!firstTrackProcessed) {
                        // 그리고 아직 첫 번째 재생 가능한 곡을 처리하지 않은 경우
                        ...
                        // 이미 첫 번째 곡을 처리한 후라면, 추가 재생은 하지 않고 로그만 남김
    ...
    private void playTrack(String trackUri, String trackName) {
        ...
        mSpotifyAppRemote.getPlayerApi().play(trackUri).setResultCallback(empty -> {
        ...
        SpotifyAppRemote.connect(this, connectionParams, new Connector.ConnectionListener() {
            ...
            public void onFailure(Throwable throwable) {
                ...
                if (throwable instanceof com.spotify.android.appremote.api.error.CouldNotFindSpotifyApp) {
                    new AlertDialog.Builder(ResultActivity.this).setTitle("Spotify 앱 미설치").setMessage("음악을 재생하려면 Spotify 앱이 필요합니다. 지금 설치하시겠습니까?").setPositiveButton("설치하기", (dialog, which) -> {
                                try {
                            ...
                            }).setNegativeButton("나중에", (dialog, which) -> Toast.makeText(ResultActivity.this, "Spotify 앱 설치 후 이용해주세요.", Toast.LENGTH_LONG).show()).setIcon(android.R.drawable.ic_dialog_info) // 아이콘 설정 (선택 사항)
